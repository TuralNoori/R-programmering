...
query_list <- list(
  query = list(
    ...
    list(
      ...
      selection = list(
        filter = "item",
        values = as.list(as.character(2002:2024))
      )
    )
  ),
  response = list(
...
# Convert query to JSON and fetch data
query_json <- toJSON(query_list, auto_unbox = TRUE)
px_data <- pxweb_get(
  url = "https://api.scb.se/OV0104/v1/doris/en/ssd/TK/TK1001/TK1001A/FordonTrafik",
  query = query_json
)
...
# Fix column names
colnames(px_data_frame) <- c("Region", "VehicleType", "Year", "Vehicles")

# Clean and convert types
px_data_frame <- px_data_frame %>%
  mutate(
    Year = as.integer(Year),
    Vehicles = as.numeric(gsub(" ", "", Vehicles))  # Remove thousand-separators
  )

# Get latest year
latest_year <- max(px_data_frame$Year, na.rm = TRUE)

# Filter for the latest year
latest_data <- filter(px_data_frame, Year == latest_year)

# Summera alla regioner per år
total_per_year <- px_data_frame %>%
  group_by(Year) %>%
  summarise(TotalVehicles = sum(Vehicles, na.rm = TRUE))

# Plot: Linjär regression över tid för hela Sverige
ggplot(total_per_year, aes(x = Year, y = TotalVehicles)) +
  geom_point(color = "darkgreen", size = 2) +
  geom_smooth(method = "lm", color = "blue", se = TRUE) +
  labs(
    title = "Linjär regression: Totalt antal fordon i Sverige över tid",
    x = "År",
    y = "Antal fordon"
  ) +
  theme_minimal()
